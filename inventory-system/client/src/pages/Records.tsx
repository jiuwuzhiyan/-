...
import { Table, Input, DatePicker, Select, Space, message, Modal } from 'antd'
...
const Records = ({ user }: RecordsProps) => {
  ...
  const getTypeTag = (type: string) => {
    const map: Record<string, { className: string; text: string }> = {
      inbound: { className: 'status-tag', text: '入库' },
      outbound: { className: 'status-tag', text: '出库' },
      return: { className: 'status-tag', text: '回库' }
    }
      ...
      render: (_: any, record: any) => (
        <Space>
          ...
          {getPrintPermission(record.type) && (
            <a onClick={() => printOrder(record)} className="action-link"><PrinterOutlined /> 打印</a>
          )}
        </Space>
      )
    }
  ...
  return (
    <div className="page-container">
      <div className="page-header animate-fade-in">
        <h1 className="page-title">出入库记录</h1>
      </div>
      ...
      <div style={{ marginBottom: 16 }}>
        <Space wrap>
          ...
          <Input
            ...
            style={{ width: 150, background: 'rgba(255,255,255,0.05)', border: '1px solid rgba(255,255,255,0.1)', color: '#e5e7eb' }}
          />
          <Input
            placeholder="操作人"
          ...
          <GradientButton icon={<SearchOutlined />} onClick={handleSearch} gradient="blue">查询</GradientButton>
          <GradientButton onClick={handleReset} gradient="purple">重置</GradientButton>
          <GradientButton icon={<ExportOutlined />} onClick={exportToExcel} gradient="green">导出Excel</GradientButton>
        </Space>
      </div>
